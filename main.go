...
func (h *handler) walkModules(m *tfjson.StateModule) error {
	for _, r := range m.Resources {
		if r.Type == "apko_build" {
			...
			configs, ok := r.AttributeValues["configs"]
			if !ok {
				continue
			}
			if configs == nil {
			...
			asMap, ok := configs.(map[string]interface{})
			if !ok {
				return fmt.Errorf("configs is %T in %s", configs, r.Address)
			}

			idx, ok := asMap["index"]
			if !ok {
				return fmt.Errorf("no index arch in %s: %v", r.Address, asMap)
			}
			_ = idx // TODO
			...
			if len(asMap) == 3 {
				arm, ok := asMap["arm64"]
				if !ok {
					return fmt.Errorf("no arm arch in %s: %v", r.Address, asMap)
				}
				amd, ok := asMap["amd64"]
				if !ok {
					return fmt.Errorf("no amd64 arch in %s: %v", r.Address, asMap)
				}

				if diff := cmp.Diff(packages(arm), packages(amd)); diff != "" {
					fmt.Printf("%s diff (-aarch64, +x86_64)\n", r.Address)
					fmt.Printf("%s\n\n", diff)
				}
			}
		}
...
func packages(in any) []string {
	asMap, ok := in.(map[string]any)
	if !ok {
		panic("wtf")
	}
	b, err := json.Marshal(asMap["config"])
	if err != nil {
